...
// node_modules/cubing/dist/lib/cubing/chunks/chunk-MA47PABE.js
  ...
  async connectedCallback() {
    ...
    if (["/edit/", "/explore/"].includes(pathname)) {
      ...
      if (config.puzzle && !(config.puzzle in puzzles)) {
        const puzzleDescription = (await import("./puzzle-geometry-RKLE7GZN.js")).getPuzzleDescriptionString(config.puzzle);
  ...
  constructor(alg = "", prevNode) {
    ...
    while (!found && tries < maxTries) {
      const testRect = {
        ...
        top: y2,
        right: x + nodeWidth,
        bottom: y2 + nodeHeight
      };
      const collision = _Node.placedRects.some(
        (r2) => !(testRect.right + _Node.margin < r2.left || testRect.left > r2.right + _Node.margin || testRect.bottom + _Node.margin < r2.top || testRect.top > r2.bottom + _Node.margin)
      );
      if (!collision) {
        found = true;
        break;
      ...
      angle += Math.PI / 6;
      radius += 8;
      x = _Node.startX + Math.round(Math.cos(angle) * radius);
      y2 = _Node.startY + Math.round(Math.sin(angle) * radius);
      tries++;
    }
    this.container.style.left = `${x}px`;
    this.container.style.top = `${y2}px`;
    _Node.placedRects.push({
      left: x,
      top: y2,
      right: x + nodeWidth,
      bottom: y2 + nodeHeight
    ...
    if (prevNode) {
      ...
      let svg = document.getElementById("node-lines-svg");
      if (!svg) {
      ...
      this._updateLine = updateLine;
      prevNode._updateLine = updateLine;
    }
    ...
    document.addEventListener("mousemove", (e) => {
      ...
      if (this.line && this._updateLine) {
        this._updateLine();
      }
      if (this.prevNode && this.prevNode._updateLine) {
        this.prevNode._updateLine();
      }
    });
...
//# sourceMappingURL=chunk-LH3AK4D5.js.map
